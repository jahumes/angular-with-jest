...
export class BackendService {
    ...
    storedUsers: User[] = [
        { id: 111, name: "Test User" },
        { id: 112, name: "Test User 2" }
    ];
    ...
    complete(ticketId: number) {
        return this._setCompletedForTicket(ticketId, true);
    }

    reopenTicket(ticketId: number) {
        return this._setCompletedForTicket(ticketId, false);
    }

    private _setCompletedForTicket(ticketId: number, completed: boolean) {
        const foundTicket = this.findTicketById(+ticketId);
        if (foundTicket) {
            return of(foundTicket).pipe(
                delay(randomDelay()),
